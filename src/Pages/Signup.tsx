import { useNavigate } from "react-router-dom";
import axios from "axios";
import { GoogleAuthProvider, getAuth, signInWithPopup } from "firebase/auth";
import firebaseApp from "../firerbase/FirebaseApp";
import { FcGoogle } from "react-icons/fc";
import { useState } from "react";
import Popup from "../Components/Popup";

const googleAuth = getAuth(firebaseApp);
const googleProvider = new GoogleAuthProvider();

const Signup = () => {
  const navigate = useNavigate();
  const [loading, setLoading] = useState<boolean>(false);

  //pop all working
  interface PopInterface {
    status: boolean;
    msg: string;
  }

  const [isPOP, setPOP] = useState<boolean>(false);
  const [popData, setPopData] = useState<PopInterface | undefined>(undefined);

  function popTimeOuter() {
    setTimeout(() => {
      setPOP(false);
      navigate("/");
    }, 3000);
    navigate("/");
  }

  //google signup

  const signupWithGoogle = async () => {
    setLoading(true);
    try {
      const result = await signInWithPopup(googleAuth, googleProvider);
      const formData = {
        name: result.user.displayName,
        email: result.user.email,
        imageUrl: result.user.photoURL,
        phoneNumber: result.user.phoneNumber,
        id: result.user.uid,
        token: await result.user.getIdToken(),
      };

      const res = await axios.post(
        `  http://localhost:8787/api/user/signup`,
        formData
      );
      localStorage.setItem("auth", res.data.token);
      localStorage.setItem("name", res.data.name);
      localStorage.setItem("email", res.data.user.email);
      localStorage.setItem("imageUrl", res.data.imageUrl);
      navigate("/");
    } catch (error) {
      setPopData({
        status: false,
        msg: "Internal server error try again after some time",
      });
      setPOP(true);
      popTimeOuter();
      setLoading(false);
    }
  };

  return (
    <div className="h-[100vh] w-[100vw] px-3 bg-blue-50 flex justify-center items-center">
      <div className=" bg-blue-50 text-center w-[500px] px-10 py-4 rounded-xl">
        <h1 className="text-[3rem] font-bold tracking-tighter">Signup</h1>
        <p className="text-md  text-gray-600 font-medium">
          Enter your information to create an account
        </p>
        <div className="text-start mt-5">
          {/* <form className="">
            <label htmlFor="fullname" className="text-lg font-medium">
              Full Name
            </label>{" "}
            <br />
            <input
              onChange={handleChange}
              className="border-2 border-gray-300 py-1 mt-2 rounded-md w-full px-2 outline-none mb-4"
              type="text"
              name="fullname"
              placeholder="Jhon"
              id="fullname"
            />
            <label htmlFor="username" className="text-lg font-medium">
              Username
            </label>{" "}
            <br />
            <input
              onChange={handleChange}
              className="border-2 border-gray-300 py-1 mt-2 rounded-md w-full px-2 outline-none mb-4"
              type="text"
              name="username"
              placeholder="jhondoe@example.com"
              id="username"
            />
            <label htmlFor="password" className="text-lg font-medium">
              Password
            </label>{" "}
            <br />
            <input
              onChange={handleChange}
              className="border-2 border-gray-300 py-1 mt-2 rounded-md w-full px-2 outline-none mb-4"
              type="password"
              name="password"
              placeholder="*********"
              id="password"
            />
            <label htmlFor="course" className="text-lg font-medium">
              Course
            </label>{" "}
            <br />
            <input
              onChange={handleChange}
              className="border-2 border-gray-300 py-1 mt-2 rounded-md w-full px-2 outline-none mb-4"
              type="text"
              name="course"
              placeholder="example bca6"
              id="course"
            />
            <button
              type="submit"
              className="w-full py-3 text-lg bg-blue-700 text-white font-semibold  rounded-xl"
            >
              {loading ? "Wait ..." : "Sign Up"}
            </button>
          </form> */}
        </div>
        <button
          onClick={signupWithGoogle}
          type="submit"
          className="w-full py-3 text-lg bg-white shadow-sm my-4 border-2 font-semibold  rounded-xl flex justify-center items-center gap-3"
        >
          {loading ? (
            <svg
              aria-hidden="true"
              className="w-8 h-8 text-gray-200 animate-spin  fill-blue-600"
              viewBox="0 0 100 101"
              fill="none"
              xmlns="  "
            >
              <path
                d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                fill="currentColor"
              />
              <path
                d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                fill="currentFill"
              />
            </svg>
          ) : (
            <>
              <FcGoogle className="text-2xl" />
              Signup with google
            </>
          )}
        </button>
        <p className="mt-5">
          Already have an account ?{" "}
          <span
            onClick={() => {
              navigate("/signin");
            }}
            className="underline cursor-pointer"
          >
            Login
          </span>
        </p>
      </div>
      {isPOP && popData && (
        <div className="fixed h-[60px] w-[300px] z-[200] top-[10%] right-0">
          <Popup isRight={popData.status} message={popData.msg} />
        </div>
      )}
    </div>
  );
};

export default Signup;
